...
func TestCreateRequest(t *testing.T) {
	t.Run("test empty student id", func(t *testing.T) {
		_, err := CreateTransferRequest("", []string{}, nil, nil)
		AssertNoError(t, err)
	})
	t.Run("test valid dorm", func(t *testing.T) {
		req, err := CreateTransferRequest(std1, []string{"dom_1"}, nil, nil)
		AssertNoError(t, err)
		AssertDeepEqual(t, req.PreferredDorms, []string{"dom_1"})
	})

	t.Run("test invalid dorm", func(t *testing.T) {
		_, err := CreateTransferRequest(std1, []string{"dom_schmit"}, nil, nil)
		AssertError(t, err)
	})
